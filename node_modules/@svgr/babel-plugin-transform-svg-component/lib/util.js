...
const getProps = ({
  ...
  if (props.length === 1 && opts.expandProps) {
    return addTypeAnotation(t.identifier('props'), typeAnnotation(getSvgPropsTypeAnnotation(t)), opts);
  }

  return addTypeAnotation(t.objectPattern(props), typeAnnotation(opts.expandProps ? intersectionTypeAnnotation([getSvgPropsTypeAnnotation(t), genericTypeAnnotation(t.identifier('SVGRProps'))]) : genericTypeAnnotation(t.identifier('SVGRProps'))), opts);
};
...
const getInterface = ({
  ...
  if (opts.ref) {
    properties.push(objectTypeProperty(t.identifier('svgRef'), genericTypeAnnotation(qualifiedTypeIdentifier(t.identifier('React'), t.identifier('Ref')), typeParameters([genericTypeAnnotation(t.identifier('SVGSVGElement'))])), true));
  }
